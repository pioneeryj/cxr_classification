"""
Training script with calibration distillation.
...
4. KL divergence loss로 모델이 calibrated distribution을 학습하도록 distillation

Loss = BCE + alpha * KL(calibrated_prob || model_prob)

Usage:
    python train_calibration_distill.py --config configs/densenet121_config.yaml
"""
...
class TemperatureScaling(nn.Module):
    ...
    def fit(self, logits, labels, masks, lr=0.01, max_iter=50, num_restarts=5):
...
class CalibrationDistillLoss(nn.Module):
    """
    Calibration distillation loss using KL divergence.
    ...
    def forward(self, logits, targets, mask):
        """
        Compute KL divergence loss between calibrated probs and model probs.

        KL(p_calibrated || p_model) for binary classification per label:
        = p_cal * log(p_cal / p_model) + (1-p_cal) * log((1-p_cal) / (1-p_model))
        ...
        with torch.no_grad():
            ...
            scaled_logits = logits.detach() / temperatures
        ...
        kl = p_cal * (torch.log(p_cal) - torch.log(p_mod)) + \
             (1 - p_cal) * (torch.log(1 - p_cal) - torch.log(1 - p_mod))

        # Apply mask and compute mean
        masked_kl = kl * mask

        if mask.sum() > 0:
            loss = self.alpha * masked_kl.sum() / mask.sum()
        else:
...
class TrainerCalibrationDistill:
    """Trainer with calibration distillation."""

    def __init__(self, config, resume=True, checkpoint_dir=None, load_checkpoint=None, start_epoch=None, weight_prefix=None):
        self.config = config
        self.resume = resume
        ...
        self.alpha_kl = self.calibration_config.get('alpha_kl', 1.0)
        self.distill_lr_factor = self.calibration_config.get('distill_lr_factor', 0.1)  # LR reduction when distillation starts
        self.kl_rampup_epochs = self.calibration_config.get('kl_rampup_epochs', 5)  # Epochs to ramp up KL loss
        self.pos_weight_decay = self.calibration_config.get('pos_weight_decay', False)
        self.current_alpha_kl = 0.0  # Will be updated during training
        ...
        wandb.init(
            ...
            name=f"{config['model']['name']}_calib_distill_fold{config['data'].get('fold', 0)}",
            config=config,
            dir=self.output_dir,
            tags=['calibration_distill', 'kl_divergence'],
        )
        ...
        self.criterion_kl = CalibrationDistillLoss(
    ...
    def _setup_criterion(self):
        ...
        if use_pos_weight:
            ...
            self.original_pos_weight = pos_weight.clone()
    ...
    def _update_calibration(self):
        ...
        # Update KL loss with new temperatures
        self.criterion_kl.update_temperatures(t_global, t_label_wise, use_label_wise)

        print("\nCalibration temperatures updated for KL distillation.")
    ...
    def _decay_pos_weight(self, epoch):
        """Linearly decay pos_weight from original toward 1.0 after warmup."""
        if self.original_pos_weight is None:
            return

        epochs_since_warmup = epoch - self.warmup_epochs
        total_distill_epochs = self.config['training']['num_epochs'] - self.warmup_epochs

        if total_distill_epochs <= 0:
            return

        # ratio: 0.0 at warmup start -> 1.0 at last epoch
        ratio = min(epochs_since_warmup / total_distill_epochs, 1.0)

        # Linear interpolation: original_pos_weight -> 1.0
        new_pos_weight = self.original_pos_weight * (1.0 - ratio) + 1.0 * ratio
        self.criterion_bce.pos_weight = new_pos_weight

        print(f"\n[pos_weight decay] epoch {epoch}: ratio={ratio:.3f}, "
              f"pos_weight range=[{new_pos_weight.min():.3f}, {new_pos_weight.max():.3f}]")

    def _get_kl_rampup_weight(self, epoch):
        """
        Get KL loss weight with linear ramp-up.

        Args:
            epoch: Current epoch

        Returns:
            Current alpha_kl value (ramped up)
        """
        ...
        if self.kl_rampup_epochs <= 0:
            # No ramp-up, use full weight immediately
            return self.alpha_kl

        if epochs_since_distill >= self.kl_rampup_epochs:
            # Ramp-up complete
            return self.alpha_kl

        # Linear ramp-up: 0 -> alpha_kl over kl_rampup_epochs
        # epochs_since_distill=0일 때 0부터 시작, kl_rampup_epochs-1일 때 full
        rampup_ratio = epochs_since_distill / self.kl_rampup_epochs
        return self.alpha_kl * rampup_ratio
    ...
    def train(self):
        ...
        print(f"")
        print(f"  [Calibration Distillation]")
        print(f"  Warmup epochs (BCE only): {self.warmup_epochs}")
        print(f"  Temperature update frequency: every {self.t_update_freq} epochs")
        print(f"  Loss = BCE + alpha_kl * KL(calibrated || model)")
        print(f"  Alpha KL (target): {self.alpha_kl}")
        print(f"  KL ramp-up epochs: {self.kl_rampup_epochs}")
        print(f"  pos_weight decay after warmup: {self.pos_weight_decay}")
        ...
        for epoch in epoch_bar:
            ...
            if use_distillation:
                ...
                if epochs_since_warmup == 0:
                    # First distillation epoch: fit T once, reduce LR
                    self._reduce_lr_for_distillation()
                    self._update_calibration()
                elif self.t_update_freq > 0 and epochs_since_warmup % self.t_update_freq == 0:
                    self._update_calibration()

                # Decay pos_weight toward 1.0
                if self.pos_weight_decay:
                    self._decay_pos_weight(epoch)

                # Update KL weight with ramp-up
                self.current_alpha_kl = self._get_kl_rampup_weight(epoch)
                self.criterion_kl.alpha = self.current_alpha_kl

            # Train for one epoch
            train_loss, bce_loss, kl_loss = self.train_epoch(use_distillation)
            ...
            # Log metrics
            self.log_epoch_metrics(epoch, train_loss, bce_loss, kl_loss, val_metrics)
    ...
    def train_epoch(self, use_distillation=False):
        ...
        if show_progress:
            status = "BCE+KL" if use_distillation else "BCE only"
            iter_bar = tqdm(iter_bar, desc=f'Epoch {self.current_epoch} ({status})', leave=False)

        for batch_idx, batch in enumerate(iter_bar):
            loss, bce_loss, kl_loss = self.train_step(batch, use_distillation)

            if loss is not None:
                epoch_loss += loss
                epoch_bce_loss += bce_loss
                epoch_kl_loss += kl_loss if kl_loss else 0

                self.iter_meter.update(
                    loss=loss,
                    bce_loss=bce_loss,
                    kl_loss=kl_loss if kl_loss else 0,
                    epoch=self.current_epoch
                )
                ...
                wandb.log({
                    ...
                    'train/kl_loss_iter': kl_loss if kl_loss else 0,
        ...
        return epoch_loss / n_batches, epoch_bce_loss / n_batches, epoch_kl_loss / n_batches
    ...
    def train_step(self, batch, use_distillation=False):
        ...
        loss, bce_loss, kl_loss = outputs

        self.scaler.scale(loss).backward()
        self.scaler.step(self.optimizer)
        self.scaler.update()

        return loss.item(), bce_loss.item(), kl_loss.item() if kl_loss is not None else None
    ...
    def forward_batch(self, batch, use_distillation=False):
        ...
        # KL Distillation Loss (after warmup)
        if use_distillation:
            kl_loss = self.criterion_kl(logits, Y, Ymask)
    ...
    def save_checkpoint(self, epoch):
        ...
        if self.weight_prefix:
            ...
            model_name = self.config['model']['name']
            prefix = f'{model_name}_calib_distill_kl{self.alpha_kl}'
        checkpoint_path = os.path.join(weight_dir, f'{prefix}_{epoch}ep.pt')
        ...
        checkpoint = {
            ...
            'temperatures': self.criterion_kl.temperatures.cpu().tolist(),
        }
        torch.save(checkpoint, checkpoint_path)
        print(f"Saved checkpoint to: {checkpoint_path}")

    def log_epoch_metrics(self, epoch, train_loss, bce_loss, kl_loss, val_metrics):
        ...
        wandb_log = {
            ...
            'train/alpha_kl': self.current_alpha_kl if use_distillation else 0.0,
        ...
        if use_distillation:
            status = f"YES (alpha_kl={self.current_alpha_kl:.3f})"
        else:
            status = "NO (warmup)"
        print(f"\nEpoch {epoch}:")
        print(f"  Train Loss: {train_loss:.4f} (BCE: {bce_loss:.4f}, KL: {kl_loss:.4f})")
...
def main():
    parser = argparse.ArgumentParser(
        description='Train CXR model with calibration distillation'
    )
    ...
    parser.add_argument('--alpha_kl', type=float, default=1.0, help='KL distillation loss weight')
    ...
    parser.add_argument('--kl_rampup_epochs', type=int, default=5,
                        help='Epochs to ramp up KL loss from 0 to alpha_kl (default: 5)')
    parser.add_argument('--pos_weight_decay', type=str, default='false', choices=['true', 'false'],
                        help='Decay pos_weight toward 1.0 after warmup (default: false)')
    ...
    parser.add_argument('--weight-prefix', type=str, default=None,
                        help='Checkpoint filename prefix (e.g., resnet50_calib_v2). Default: {model}_calib_distill_kl{alpha_kl}')
    ...
    config['calibration'] = {
        ...
        'kl_rampup_epochs': args.kl_rampup_epochs,
        'pos_weight_decay': args.pos_weight_decay.lower() == 'true',
    }
    ...
    trainer = TrainerCalibrationDistill(
        config,
        resume=resume,

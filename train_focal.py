"""
Training script with BCE + Focal loss for calibration.

Loss = BCE(logits, y) + alpha_focal * FocalLoss(logits, y)

Focal loss is applied uniformly across all labels after warmup epochs.

Usage:
    python train_focal.py --config configs/resnet50_config.yaml
"""
...
class FocalLoss(nn.Module):
    ...
    def __init__(self, alpha=0.1, gamma=2.0):
        """
        Args:
            alpha: Focal loss weight (multiplied to the final loss)
    ...
    def forward(self, logits, targets, mask):
        ...
        Returns:
            focal_loss: scalar tensor (alpha * focal)
        """
        ...
        if mask.sum() > 0:
            loss = self.alpha * masked_focal.sum() / mask.sum()
        else:
...
class TrainerFocal:
    """Trainer class with BCE + Focal calibration loss."""
    ...
    def __init__(self, config, checkpoint_dir=None,
        ...
        self.alpha_focal = self.calibration_config.get('alpha_focal', 0.1)
        self.gamma = self.calibration_config.get('gamma', 2.0)
        ...
        wandb.init(
            ...
            name=f"{config['model']['name']}_focal_fold{config['data'].get('fold', 0)}",
            config=config,
            dir=self.output_dir,
            tags=['focal', 'calibration'],
        )
        ...
        self.criterion_focal = FocalLoss(alpha=self.alpha_focal, gamma=self.gamma)
    ...
    def train(self):
        ...
        print(f"  Focal loss starts at epoch: {self.warmup_epochs}")
        print(f"  Alpha Focal: {self.alpha_focal}")
        ...
        if self.start_epoch >= num_epochs:
            ...
            for epoch in epoch_bar:
                ...
                train_loss, bce_loss, focal_loss = self.train_epoch()
                ...
                self.log_epoch_metrics(epoch, train_loss, bce_loss, focal_loss, val_metrics)
    ...
    def train_epoch(self):
        ...
        if show_progress:
            calib_status = "BCE+Focal" if use_calibration else "BCE only"
            iter_bar = tqdm(iter_bar, desc=f'Epoch {self.current_epoch} ({calib_status})', leave=False)

        for batch_idx, batch in enumerate(iter_bar):
            loss, bce_loss, focal_loss = self.train_step(batch, use_calibration)

            if loss is not None:
                epoch_loss += loss
                epoch_bce_loss += bce_loss
                epoch_focal_loss += focal_loss if focal_loss else 0

                self.iter_meter.update(
                    loss=loss,
                    bce_loss=bce_loss,
                    focal_loss=focal_loss if focal_loss else 0,
                ...
                wandb.log({
                    ...
                    'train/focal_loss_iter': focal_loss if focal_loss else 0,
        ...
        avg_focal = epoch_focal_loss / n_batches

        return avg_loss, avg_bce, avg_focal
    ...
    def train_step(self, batch, use_calibration=False):
        ...
        loss, bce_loss, focal_loss = outputs

        self.scaler.scale(loss).backward()
        self.scaler.step(self.optimizer)
        self.scaler.update()

        return loss.item(), bce_loss.item(), focal_loss.item() if focal_loss is not None else None
    ...
    def forward_batch(self, batch, use_calibration=False):
        ...
        # Focal Loss (only after warmup)
        if use_calibration:
            focal_loss = self.criterion_focal(preds, Y, Ymask)
            loss = bce_loss + focal_loss
        else:
            focal_loss = None
            loss = bce_loss

        return loss, bce_loss, focal_loss
    ...
    def save_checkpoint(self, epoch):
        ...
        checkpoint_path = os.path.join(weight_dir, f'{model_name}_focal{alpha_focal}_{epoch}ep.pt')
    ...
    def log_epoch_metrics(self, epoch, train_loss, bce_loss, focal_loss, val_metrics):
        ...
        print(f"\nEpoch {epoch}:")
        print(f"  Train Loss: {train_loss:.4f} (BCE: {bce_loss:.4f}, Focal: {focal_loss:.4f})")
...
def main():
    parser = argparse.ArgumentParser(
        description='Train CXR classification model with BCE + Focal calibration loss'
    )
    parser.add_argument('--config', type=str, required=True, help='Path to configuration YAML file')
    parser.add_argument('--num_epochs', type=int, default=None,
                        help='Total training epochs (overrides config)')
    parser.add_argument('--warmup_epochs', type=int, default=5,
                        help='Warmup epochs (BCE only, Focal starts after this)')
    parser.add_argument('--alpha_focal', type=float, default=0.1,

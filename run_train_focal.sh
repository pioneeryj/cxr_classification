...
# BCE + Focal Loss 학습 스크립트
# 체크포인트에서 이어서 Focal loss 적용
#
# 사용법:
#   ./run_train_focal.sh
...
# 모델 설정
MODEL="BiomedCLIP"                      # densenet121, resnet50, biovil, medklip, medclip
GPU=0
...
# Focal Loss 설정
WARMUP_EPOCHS=5                        # warmup epochs (이후부터 Focal loss 시작)
ALPHA_FOCAL_LIST="1.0 2.0"        # 실험할 alpha_focal 값들
...
# Alpha Focal 값별로 실험
for ALPHA_FOCAL in $ALPHA_FOCAL_LIST; do
    OUTPUT_DIR="${OUTPUT_BASE}/${MODEL}_focal${ALPHA_FOCAL}"

    echo ""
    echo "========================================"
    echo "BCE + Focal Loss 학습 (alpha_focal=${ALPHA_FOCAL})"
    echo "========================================"
    echo "모델: $MODEL"
    echo "Config: $CONFIG"
    ...
    echo ""
    echo "$MODEL alpha_focal=${ALPHA_FOCAL} 학습 완료!"
    echo ""
done

echo "========================================"
echo "모든 alpha_focal 실험 완료!"
echo "========================================"
